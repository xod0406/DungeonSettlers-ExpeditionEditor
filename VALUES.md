...
The unfinished enum values `Logging`, `Mining`, `Cooking`, `Crafting`, `Construction`, and `Unbreakable` are intentionally hidden from the picker.
...
For individual grades, use `LockTalents=true` for the slot and disable the global all-Genius option.
